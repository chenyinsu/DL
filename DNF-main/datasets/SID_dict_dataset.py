...
class BaseDictSet(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if not self.load_npy:
            ...
            # print(f"final path: {os.path.join(self.data_path, img_file.replace('short', 'short_pack') + '.npy')}")
            # 规范化 img_file，去除多余的 ./ 等
            clean_img_file = os.path.normpath(img_file)  # 例如 'Sony/short/00206_02_0.04s.ARW'

            # 替换 short -> short_pack
            clean_img_file = clean_img_file.replace('short', 'short_pack')

            # 拼接完整路径
            final_path = os.path.join(self.data_path, clean_img_file + '.npy')

            # 加载 npy 文件
            noisy_raw = np.load(final_path, allow_pickle=True)
