...
# from .dnf_utils.Freq import DualGateFreqSplit
...
class DNF(DNFBase):
    def __init__(self, f_number, *,
        ...
        #     DualGateFreqSplit(in_channels=f_number * (2 ** i)) for i in range(layers)
        # ])
        self.local_module = nn.ModuleList([
            Local(in_channels=f_number * (2 ** i)) for i in range(layers)
        ])
    ...
    def _pass_features_to_color_decoder(self, x, f_short_cut, encoder_features):
        ...
        # **计算残差信息**
        # residual = x - f_short_cut  # 计算去噪结果与 shortcut 之间的残差
        ...
        x = f_short_cut
        for fuse, denoise, down, local, globals, decoder_feedback_feature in zip(
                self.feedback_fuses[:-1],
                self.denoising_blocks[:-1],
                self.downsamples,
                self.local_module[:-1],
                self.global_module[:-1],
            ...
            x_local = local(x)
            x_global = globals(x)
            encoder_x_local.append(x_local)  # 低频信息
            encoder_x_global.append(x_global)  # 高频信息
            x = down(x)
        ...
        return res1, encoder_features, x_local, x_global, encoder_x_local, encoder_x_global
