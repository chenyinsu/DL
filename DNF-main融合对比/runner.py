...
def main(config):
    ...
    if config['train'].get('pretrained') and (not config['train'].get('resume')):
        ...
        load_pretrained(
            ...
            model,
            logger,
            load_modules=["feature_conv_0", "feature_conv_1", "denoising_blocks", "downsamples", "aux_denoising_blocks", "aux_upsamples",
                          "denoising_decoder_fuses", "aux_conv_fuse_0", "aux_conv_fuse_0","feedback_fuses"]
        )
        validate(config, test_forward, model, loss_list, valid_dataloader, config['train'].get('start_epoch', 0), writer)
        if config.get('eval_mode', False):
            return
...
def validate(config, test_forward, model, loss_list, data_loader, epoch, writer):
    ...
    for idx, data in enumerate(data_loader):
        ...
        if config['testset_as_validset']:
            ...
            # psnr, ssim, lpips = test_metric_cuda(config, epoch, outputs[config['test']['which_stage']], targets[config['test']['which_gt']], img_files, lbl_files)
        ...
        if config['testset_as_validset'] or idx % config['print_per_iter'] == 0 or idx == len(data_loader):
            ...
            logger.info(
                ...
                # f'LPIPS {lpips_meter.val:.4f} ({lpips_meter.avg:.4f})\t'
...
def extract_texture_map(img):
    """使用Sobel算子提取纹理（边缘）信息"""
    gray = img.mean(dim=1, keepdim=True)  # 转灰度
    ...
    return edge_mag / edge_mag.max().clamp(min=1e-6)  # 归一化到0~1
...
def test_metric_cuda(config, epoch, outputs, targets, image_paths, target_params=None):
    ...
    if config['test']['save_image']:
        ...
        texture = extract_texture_map(outputs / 255.0)  # 注意缩放到 [0, 1] 再提纹理
        texture_save_path = os.path.join(result_path, f'{os.path.basename(image_paths[0])[:-4]}_{psnr.item():.2f}_texture.png')
        save_image_torch(texture[0], texture_save_path)
...
def test_metric_cuda(config, epoch, outputs, targets, image_paths, target_params=None):
    ...
    # === LPIPS ===
    with torch.no_grad():
        outputs_lpips = torch.clamp(outputs_raw, 0, 1)
        targets_lpips = torch.clamp(targets_raw, 0, 1)
        lpips_value = loss_fn_lpips(outputs_lpips, targets_lpips).mean()
    ...
    if config['test']['save_image']:
        ...
        texture = extract_texture_map(outputs / 255.0)
        texture_save_path = os.path.join(result_path, f'{os.path.basename(image_paths[0])[:-4]}_{psnr.item():.2f}_texture.png')
        save_image_torch(texture[0], texture_save_path)
